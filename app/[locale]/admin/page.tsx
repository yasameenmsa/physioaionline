...
import { HelpCircle, BookOpen, Users, Layers, Rss, GraduationCap, Calendar, Eye, DollarSign } from 'lucide-react';
...
export default async function AdminDashboard({ params }: PageProps) {
  ...
  const [totalQuestions, totalCategories, totalUsers, totalNews, totalCourses, recentQuestions, recentNewsItems, recentCourses] = await Promise.all([
                  ...
                  <div key={item._id.toString()} className="flex items-center justify-between p-4 text-sm">
                    <div className="flex-1 min-w-0 mr-4">
                      <div className="flex items-center gap-2">
                        ...
                        {!item.published && (
                          <span className="text-xs bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200 px-1.5 py-0.5 rounded shrink-0">Draft</span>
                        )}
                      </div>
